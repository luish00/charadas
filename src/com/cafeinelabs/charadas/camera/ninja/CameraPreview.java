package com.cafeinelabs.charadas.camera.ninja;
...
//		NowCamera.stopPreview();//fixed for Samsung S2
		if(cameraNow==null){
			cameraNow = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT);
			Parameters parameters;
			
			parameters = cameraNow.getParameters();
			// Set the take picture size, you can set the large size of the camera supported.
			parameters.setPictureSize(PreviewSizeWidth, PreviewSizeHeight);
			
			cameraNow.setParameters(parameters);
		}
		
		cameraNow.takePicture(shutterCallback, rawPictureCallback, jpegPictureCallback);
